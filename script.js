...
// Dirubah 
const pengirim = "Yusuf";
const nomorWa = "6287840230139" // awalan nomor 0 harus di awalin 62
const textWa = "Ah bisa aja kamu"
const pesan = `Your name is like a melody that always lingers in my mind.`;

if (pengirim) {
  nama.innerHTML = pengirim;
  nama1.innerHTML = pengirim;
} else {
  nama.innerHTML = "Yusuf";
  nama1.innerHTML = "Yusuf";
}
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'Halo, isi dengan nama lengkapmu dong',
    ...
    preConfirm: () => {
      ...
      if (!login) {
        Swal.showValidationMessage(`Harus di isi ya cantik 😆`)
      }
      return { login }
    }
  }).then((result) => {
    ...
    let txt = `Hello ${result.value.login}, such a beatiful name. ${pesan}`;
